...
def uebernehmen_vorschau(
    ...
    uebernehmen_bestaetigen). Der Kündigungsweg (Bank+Kontoart) kommt dagegen
    nur noch aus der festen Tabelle (helpers.kuendigung_vorschlag) - keine
    KI-Websuche mehr an dieser Stelle, siehe kuendigung_recherche.
    naechtlicher_lauf() für die, die stattdessen nachts läuft."""
    gueltig = [
...
async def uebernehmen_bestaetigen(request: Request, db: Session = Depends(get_db)):
    ...
    als KWK_TIMEOUT_SEKUNDEN auf eine hängende KI-Websuche. Der
